...
int DhcpClass2::beginWithDHCPV6(uint8_t *mac, unsigned long timeout, unsigned long responseTimeout)
{
	_dhcpLeaseTime = 0;
	_dhcpT1 = 0;
	_dhcpT2 = 0;
...
void DhcpClass2::reset_DHCPV6_lease()
{
	memset(_dhcpGua, 0, 16 * 5);
}

//return:0 on error, 1 if request is sent and response is received
int DhcpClass2::request_DHCPV6_lease()
{
...
void DhcpClass2::send_DHCPV6_SOLICIT(void)
{
	uint16_t j;
	uint8_t ip[16];
...
uint8_t DhcpClass2::send_DHCPV6_REQUEST(uint8_t type)
{
	//uint16_t i;
	uint16_t j;
	uint8_t ip[16];
...
uint8_t DhcpClass2::DHCPV6_run_stateful()
{
	uint8_t type;
	uint8_t ret;
	uint8_t i;
...
uint8_t DhcpClass2::DHCPV6_run_stateless()
{
	uint8_t type;
	uint8_t ret;
...
void DhcpClass2::InitDhcpV6Option(unsigned asize, unsigned agrowby)
{
	size = asize;
	growby = agrowby;
	num = 0;
...
void DhcpClass2::DumpDhcpV6Option(char *sMark)
{
	unsigned i;
...
void DhcpClass2::AppendDhcpV6Option(uint8_t value)
{
	uint32_t need;
...
uint8_t DhcpClass2::send_DHCPV6_INFOREQ(void)
{
	uint16_t j;
	uint8_t ip[16];
	uint8_t rip_msg_size;
...
int8_t DhcpClass2::parseDHCPV6MSG()
{
	uint8_t svr_addr[16];
	uint16_t svr_port;
	uint8_t addlen;
...
void DhcpClass2::presend_DHCPV6()
{
}

int DhcpClass2::checkLease()
{
...
IP6Address DhcpClass2::getGua()
{
	return IP6Address(_dhcpGua);
}
#if 0
IP6Address DhcpClass2::getSubnetMask()
{
	return IP6Address(_dhcpSubnetMask);
}

IP6Address DhcpClass2::getGatewayIp()
{
	return IP6Address(_dhcpGatewayIp);
}

IP6Address DhcpClass2::getDhcpServerIp()
{
	return IP6Address(_dhcpDhcpServerIp);
}

IP6Address DhcpClass2::getDnsServerIp()
{
	return IP6Address(_dhcpDnsServerIp);
}
#endif

void DhcpClass2::printByte(char *buf, uint8_t n)
{
	char *str = &buf[1];
	buf[0] = '0';
	do
